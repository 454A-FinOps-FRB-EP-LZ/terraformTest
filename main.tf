...
resource "aws_launch_template" "web_lc" {
  ...
  user_data = base64encode(<<-EOF
    ...
    sudo FLASK_APP=Routes.py flask run --host=0.0.0.0 --port=80 &
...
#   period = "120"
  statistic = "Average"
  threshold = "70" # create new instance when cpu >= 75
  dimensions = {
...
#   period = "120"
  statistic = "Average"
  threshold = "30" # create new instance when cpu >= 75
  dimensions = {
    "AutoScalingGroupName" = aws_autoscaling_group.web_asg.name
  }
  actions_enabled = true
...
resource "aws_cloudwatch_metric_alarm" "memory_scale_up_alarm" {
  ...
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods = "2"
  metric_name = "mem_used_percent" # Assuming CloudWatch Agent publishes this custom metric
  namespace = "CWAgent" # Custom namespace defined in CloudWatch Agent config
#   period = "120"
  period = "10"
  statistic = "Average"
  threshold = "80" # Scale up when memory usage >= 80%
  dimensions = {
    "AutoScalingGroupName" = aws_autoscaling_group.web_asg.name
  }
  actions_enabled = true
...
resource "aws_cloudwatch_metric_alarm" "memory_scale_down_alarm" {
  ...
  comparison_operator = "LessThanOrEqualToThreshold"
  evaluation_periods = "2"
  metric_name = "mem_used_percent" # Custom metric for memory usage
  namespace = "CWAgent" # Custom namespace
#   period = "120"
  period = "10"
  statistic = "Average"
  threshold = "40" # Scale down when memory usage <= 40%
  dimensions = {
    "AutoScalingGroupName" = aws_autoscaling_group.web_asg.name
  }
  actions_enabled = true
